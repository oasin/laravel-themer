...
trait AuthScaffolding
{
    ...
    {
        return Theme::path($path, $this->slug);
    }
    ...
    {
        $this->ensureDirectoryExists(Theme::path('views/auth', $this->slug));
        $this->ensureDirectoryExists(Theme::path('views/layouts', $this->slug));

        if (is_dir(__DIR__ . "/../../../stubs/App/resources/{$this->cssFramework}/views")) {
            $this->copyDirectory(
                __DIR__ . "/../../../stubs/App/resources/{$this->cssFramework}/views/auth",
                Theme::path('views/auth', $this->slug)
            );
            $this->copyDirectory(
                __DIR__ . "/../../../stubs/App/resources/{$this->cssFramework}/views/layouts",
                Theme::path('views/layouts', $this->slug)
            );

            copy(
                __DIR__ . "/../../../stubs/App/resources/{$this->cssFramework}/views/home.blade.php",
                Theme::path('views/home.blade.php', $this->slug)
            );

            $this->replaceInFile('%theme%', $this->slug, Theme::path('views/layouts/app.blade.php', $this->slug));
        }
